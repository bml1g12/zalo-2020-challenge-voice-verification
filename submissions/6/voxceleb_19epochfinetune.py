...
disparate_entries_are_not_matches_mask = df_test_sub["duration_dif"] > 26.49

result = get_optimal_balanced_threshold(df_test_sub)
print(result)
df_test_sub["label"] = (df_test_sub["dist"] < result["threshold"]).astype(int)
print(f"Setting {len(df_test_sub.loc[disparate_entries_are_not_matches_mask, 'label'])} items "
      f"to nonmatches based on duration disparity model.")
df_test_sub.loc[disparate_entries_are_not_matches_mask, "label"] = 0
